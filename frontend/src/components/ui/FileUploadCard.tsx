...
export default function FileUploadCard({ title, icon: Icon = Upload, file, onChange, accept = '.xlsx,.xls', hint, children }: FileUploadCardProps) {
  return (
    <div className="rounded-2xl p-5" style={{ background: 'var(--s2)', border: '1px solid var(--b1)' }}>
      <h3 className="text-sm font-bold flex items-center gap-2 mb-4" style={{ color: 'var(--t0)' }}>
        <span className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ background: 'var(--s3)', border: '1px solid var(--b1)' }}>
          <Icon className="w-3 h-3" style={{ color: 'var(--em)' }} />
        </span>
        {title}
      </h3>
      <label
      ...
      >
        {file ? (
          <div className="text-center">
            <CheckCircle2 className="w-6 h-6 mx-auto mb-2" style={{ color: 'var(--em)' }} />
            <p className="text-xs font-semibold mb-0.5" style={{ color: 'var(--em-lt)' }}>File ready</p>
            <p className="text-[11px] max-w-[180px] truncate" style={{ color: 'rgba(52,211,153,0.6)' }}>{file.name}</p>
