 ...
 * Upload an IODB file, pick columns + per-column filters, preview, and generate.
 */
import { useState, useMemo } from 'react';
import { FileSpreadsheet, Upload, FileText, Download, CheckCircle2 } from 'lucide-react';
...
export default function InstrumentListGenerator() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await api.post<AnalyzeResponse>('/instrument-list/analyze', formData, {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await api.post<GenerateResponse>('/instrument-list/generate', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
  ...
  const handleDownload = async () => {
    try {
      const response = await api.get('/instrument-list/download', { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', result?.filename || 'Instrument_List.xlsx');
  ...
  // Client-side filtered preview rows based on active filters
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Instrument List Generator"
        description="Generate an instrument list from your IODB, with column selection and per-column filters."
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <FileUploadCard
          title="Instrument List Template (optional)"
          icon={FileText}
          file={templateFile}
          onChange={setTemplateFile}
          hint="If omitted, a default formatted instrument list will be generated."
        />
      </div>
      ...
      {analysis && (
        <>
          ...
          >
            ...
            {generating ? 'Generating...' : 'Generate Instrument List'}
          </Button>
        </>
      )}
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-xl font-bold text-green-800 mb-2">Instrument List Generated!</h3>
